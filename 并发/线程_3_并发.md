...
/*
 【 悲观锁 】 ——  互斥同步（阻塞同步）
 · 总是假设坏的情况，任何时刻都会有并发冲突，所以每次访问数据时，都会上锁！其他线程想要访问数据时，就会阻塞直到它获取锁！
 · 数据库外部悲观锁：synchronized、ReentrantLock
 · 数据库内部悲观锁：排他锁（写锁），是一种互斥锁，在操作数据时都需要加锁！
 	MySQL 中的 update、insert、delete 自动加上写锁，也可以显示的对 查询语句显示加锁：select ... for update


 【 乐观锁 】 ——  非阻塞同步（无锁编程 Lock-Free）
 · 总是假设最好的情况，访问数据不会上锁，只会检测是否存在并发冲突，如果没有其他线程争用共享数据，则操作数据；如果共享数据被争用，那就执行补偿措施：失败重试，直到无冲突为止！乐观锁适用于读多写少的场景，用于提高吞吐量！
 · 实现方式：CAS、版本号机制（时间戳）！
 · 数据库中提供的类似于 write_condition 机制，其实就是乐观锁的思想！在 java.util.concurrent.atomic 包下面的原子变量类就是使用了乐观锁的一种实现方式：CAS

 【 乐观锁 VS 悲观锁 】：两种锁各有优缺点，各自都有其适合的使用场景，没有更好，只有合适！
 · 乐观锁适用于 读多写少 的场景，即：冲突很少发生，此时如果频繁地阻塞锁，并且多数情况下没有意义，这会导致大量的锁开销！若不加锁，则能加大系统的吞吐量！
 
 · 悲观锁适用于 多写 的场景，一般会经常产生冲突，此时如果使用乐观锁，则会导致上层应用不断的 retry，占用 CPU 资源，反而会降低系统 性能！
*/
```
...
2. CAS 算法
   ...
   /*
    ...
    · CAS 涉及到三个操作数：（都与需要修改的 变量A 有关）
    	 ...
    	 - A 的原值 ：视为 A 的原值，用于比较
    	 - A 的更新值 ：用于更新内存中 A 的值
    	仅当 传入的原值 与 内存中查询到的现值 相等时，才会将 更新值 赋予 A。 JVM 能保证 原值 与 现值的比较，已经 变量的赋值，会是一个原子操作！一般来说，若两者不相等，则自旋，不断的尝试：构成 自旋锁！
...
/*
 · 中断
	CPU 对系统中或者系统外发生异步事件的响应，CPU 暂停正在执行的程序，保留现场后自动转去执行相应的处理程序，处理完后返回断点，继续执行被打断的程序！
	
 · 中断处理
	中断的处理由CPU+操作系统处理：处理过程：使用中断向量表！
	中断向量表 是 操作系统为每种不同的信号中断设置一个值（16进制值）存放在中断向量表中。
	当cpu接收到中断信号时，将其告知操作系统，操作系统便会去查询中断向量表，根据值调用相应的程序。
· 硬中断、软中断
*/
```
...
/*
 · 操作系统的级别：
	现在的操作系统都有两个级别：内核态、用户态。（这个cpu在硬件上就支持了）
	
	cpu的分级：
		CPU-CPL->0 1 2 3 四个级别:
		不同的指令可以跑在不同的级别上，危险的指令只能跑在0级上，级别比较高的可以直接访问硬件，为了安全，级别低的就不能访问硬件。
		
		操作系统就利用了cpu的分级特性，以linux为例，应用了其中的两个级别0级和3级。
			linux的内核态：0级别：内核可以访问所有的内存空间
			linux的用户态：3级别：用户只能访问用户的内存空间
		linux这样的的分级使得系统非常稳定安全。
*/
```
...
```java
/*
 · 用户态调用任何内核函数（socket网络上的读写、启动、唤醒、阻塞、杀死线程...）时都会产生一个中断：著名的“80中断（0x80）”。
 · app应用经系统调用内核函数时，会发生如下步骤：
 
 	1、app发出0x80中断（以前的CPU：通过软件产生软中断80中断） 或者 调用sysenter原语（最新的CPU，在硬件上做的支持，相当于在汇编上直接有这个原语，通过这个原语就可以直接切换到内核态）
 	
 	2、os进入内核态，需要查找处理80中断的函数，同时保存硬件和app现场：
 		（1）在中断向量表中查找处理例程（去寻找哪个函数处理这个中断值0x80：就是system_call）
 		（2）保存硬件现场：CS、IP等寄存器的值
 		（3）保存app现场：堆栈和寄存器中的值
 	3、执行中断例程system_call
 		（1）根据参数与编号寻找对应请求的处理例程
 		（2）执行并返回，将返回值放在eax寄存器中
 	4、返回用户态，恢复现场：
...
+ synchronized 是一种悲观锁，它解决了 共享资源 线程安全的问题！它能实现互斥同步，保证任意时刻只有一个线程能够操作共享资源，而获取不到资源的线程则被阻塞，因此也称为 阻塞同步！
+ synchronized 本质上是通过 底层操作系统的 Mutex Lock 实现的，使得 在synchronized 上阻塞或者唤醒线程都需要操作系统的帮助！这需要通过 80 中断，使线程由用户态切换到内核态，此过程较为耗时，所以早期的 synchronized 效率低下，是一个完全的重量级锁！
...
```java
/*
 · 有 JVM规范可知，synchronized 在 JVM 中是基于 进入和退出 Monitor对象 来实现方法同步和代码块同步的！
...
+ 轻量级 是针对 操作系统 Mutex Lock实现的传统锁而言的！它的出现并不是取代重量级锁，其初衷是在没有多线程竞争资源的前提下，减少重量级锁使用 操作系统互斥量 产生的性能消耗！
...
+ **synchronized 依赖于 JVM 实现**，线程获取锁后，再执行完同步块/方法 后能自动解锁！**ReetrantLock 是 java 语言层的实现**，通过 `lock/tryLock/lockInterruptibly`等方法实现加锁，解锁时需要通过` unlock` 方法解锁，并且为保证能安全解锁，应该在` try实体中加锁，并在finally实体中解锁`！
...
+ ReentrantLock 比 synchronized 更多功能
  ...
  3. **可绑定多个条件**！
     ...
     synchronized 同步块内，使用锁对象的 wait / notify / notifyAll 实现的是 一个隐含条件下的 等待通知机制，此时所有线程都会进入一个等待队列中！要像多关联一个条件时，就必须多加一个不同的对象锁！
...
public class ThreadLocal<T> {
   ...
   static final class SuppliedThreadLocal<T> extends ThreadLocal<T> {
    ...
    static class ThreadLocalMap {
        ...
        private Entry[] table;
        // 数组初识长度：2的幂次：使得 条目 均匀分散到 entry数组中！
