...
=》HashMap 为何线程不安全？
 ...
 · 由于 HashTable、SynchronizedMap 都是对当个操作 put、get、remove等添加了锁，所以单个操作能保证线程安全，但是多个操作进行复合操作时，线程安全性仍然无法自然保障！
...
=》【 fail-fast策略 】快速失败：
 ...
 modCount 用于并发迭代（foreach、iterator）的时候，检测 集合是否有被其他线程修改过！
	...
	迭代过程中 当迭代器 使用 next() 与 hasNext() 方法时，会对比 modCount（现值） 与 expectedModCount（原值）判断所在集合是否被其他线程修改，如果两者不一致，则判断未被修改过，抛出异常：ConcurrentModificationException
...
final V putVal(K key, V value, boolean onlyIfAbsent) {
    ...
    for (Node<K,V>[] tab = table;;) {
        ...
        // 1、如果 hash码 所在桶 为null，记录桶的头结点为 f
            ...
            // cas 操作 添加node结点（该桶的头结点） ，失败则自旋：保证成功！
...
2、寻址相应的桶，判断桶是否为 null
	true：通过 cas 将结点插入，如果插入失败，则自旋，保证插入成功
break
...
跳表：
 ...
 · 多级索引跳表
 	当 索引链也变得非常长时，就使用刚才的思想，在索引链上提取出新的索引链，如此往复在新索链提取出索引链，直到 时间复杂度达到 O(longN)！
 	此时，形成的跳表就是多级索引跳表！
