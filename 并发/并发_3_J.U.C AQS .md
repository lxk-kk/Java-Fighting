...
/*
 · 核心思想：
 	AQS 核心思想是，如果当前线程请求的共享资源空闲，则将当前其设置为有效的工作线程，并将共享资源设置为锁定状态。如果请求的共享资源被占用，那就需要一套 线程阻塞 以及 线程被唤醒时 的锁分配机制！	
...
/*
 · 模板方法模式：
 	模板方法模式 与 实现接口 的方式有很大的区别！模板方法模式是基于“继承”的，主要是为了在不改变模板结构的前提下在子类中重新定义模板中的内容以实现服用代码。
 	如下例：
 ...
 · 同步器的设计是基于模板方法模式的！由上述 tryAcquire 方法即可看出！
	...
	2、讲 AQS 组合在自定义同步组件的实现中，并调用其模板方法，而这些模板方法会调用开发者重写的方法！
