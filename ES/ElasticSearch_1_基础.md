...
2. ElasticSearch
   ...
   EalsticSearch 是分布式的 文档存储引擎、全文搜索引擎、分析引擎，支持 PB（1000 TB） 级别的数据！

   它基于 Lucene 实现的全文搜索系统，自身带有分布式 的 协调管理功能，可实现 近实时搜索！

   它隐藏了 Lucene 的复杂性，并提供了简单的 restful / Java API 

3. Solr

   与 ElasticSearch 相似的全文搜索系！
...
+ 结点 表示 集群服务中的单个服务器，用于存储数据，并参与整个集群的索引和检索！
+ 结点有唯一的名称标识，通常使用 uuid 唯一标识，如果不自定义结点名称，则会在结点启动时，自动分配名称！
+ 通过集群名称可以讲结点加入指定的集群，默认会加入名为”elasticsearch“的集群！
...
+ 单台机器无法存储大量数据，于是 ES 将一个索引中的数据切分为多个 shard，分布在多台服务器上存储。
  ...
  有了 shard 就可以横向扩展，存储更多的数据，让搜索和分析等操作分不到多台服务器上完成，这样能提高 性能和吞吐量！
...
+ ELK 日志分析系统
  ...
  logstash 采集业务系统中的日志，存储到 es 中，通过 kibana展现给运维人员分析！
