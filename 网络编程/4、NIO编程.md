...
+ NIO（New IO、Unblocking IO）：同步非阻塞 IO
...
+ NIO 本身是基于 **事件驱动思想** 来完成的，主要解决的就是 BIO 模型下的大并发问题。

  ```
   · NIO 基于 Reactor，当 socket 有流可读或者 可写入 socket 时，操作系统会相应的通知应用程序进行处理，应用再将流读取到缓冲区或者写入操作系统。
   
   · 也就是说，一个 socket 连接并不一定拥有一个处理线程与之对应。如果 socket 连接中没有数据，那么并不会启动一个线程处理这个连接。
  ```

+ NIO 模型中：一个连接创建之后，不需要对应一个线程，这个连接会被注册到 *多路复用器* 上——**所有的连接都通过一个线程轮询检测**。

  ```
   · 多路复用器会 轮询检测 所有注册的连接，如果发现连接上有请求（有数据可读写），就会开启一个线程进行处理。
  ```
